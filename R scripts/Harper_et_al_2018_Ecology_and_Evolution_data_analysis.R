...
p <- c("ggplot2","grid","gridExtra","lme4","glmmADMB","MASS","car",
       "scales","AICcmodavg","gtools","xlsx","reshape2","plyr","dplyr",
       "arm","RVAideMemoire","ResourceSelection","bbmle","RColorBrewer",
       "MuMIn")
...
## Plot heatmaps for each vertebrate group
## Fish
...
#'
#' Next, examine strength of agreement between the eDNA approaches.
#' Use kappa coefficient, a test which has been performed in several 
#' eDNA papers comparing eDNA to conventional survey methods.
#' 
...
## Merge dataframe with sample metadata
...
## Subset data frame for qPCR NT positive and negative samples
qPCR_NT_neg <- subset(new.df, new.df$qPCR_NT == "N")
qPCR_NT_pos <- subset(new.df, new.df$qPCR_NT == "P")

## Find maximum number of reads for qPCR NT negative and positive samples 
## with and without GCN sequence threshold
...
## Subset data frame for qPCR TA positive and negative samples
qPCR_TA_neg <- subset(new.df, new.df$qPCR_TA == "N")
qPCR_TA_pos <- subset(new.df, new.df$qPCR_TA == "P")

## Find maximum number of reads for qPCR TA negative and positive samples 
## with and without GCN sequence threshold
...
## Make all qPCR scores > 0 positive
average.df$qPCR_NT <- factor(ifelse(average.df$qPCR_score == "0", "N", "P"))

## Make all qPCR scores > 3 positive
...
## Run custom function to statistically test for model overdispersion 
...
## PLOT 11: qPCR score and proportion GCN reads 
...
## PLOT 12: eDNA concentration and proportion GCN reads
